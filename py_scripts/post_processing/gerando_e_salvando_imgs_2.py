...
if str(resp) == 'yes' or str(resp) == 'sim' or str(resp) == 's' or str(resp) == 'y':
    ...
    pred_file = os.path.join(LOAD_DIR,'X_hat_jamc_t+5_a3_2.hkl')
    obs_file = os.path.join(LOAD_DIR,'X_test_jamc_t+5_a3_2.hkl')
...
print pred.shape
print obs.shape
resp = raw_input('deseja mudar a pasta de salvamento das imagens? Pasta padrao, prediction_plots_artigo_jamc_finetunned_3: ')
...
if str(resp) == 'yes' or str(resp) == 'sim' or str(resp) == 's' or str(resp) == 'y':
    ...
    SAVE_DIR = DATA_DIR+'prediction_plots_artigo_jamc_finetunned_3'
...
for i in range(80):
    for t in range (nt): 
        ...
        plot.savefig(SAVE_DIR+'/'+'pred_'+str(i)+'_'+str(t)+extensao,dpi=200,bbox_inches='tight')
...
for i in range(80):
    for t in range (nt): 
        ...
        plot.ylim(ylim)   
        plot.tick_params(axis='both', which='both', bottom='off', top='off', left='off', right='off', labelbottom='off', labelleft='off')
        plot.savefig(SAVE_DIR+'/'+'obs_'+str(i)+'_'+str(t)+extensao,dpi=200,bbox_inches='tight')
