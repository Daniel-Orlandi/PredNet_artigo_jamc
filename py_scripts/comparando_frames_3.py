...
DATA_DIR='/home/daniel/PredNet_artigo_jamc/kitti_results/prediction_plots_mono/'
...
for i in range(len(obs_pred[0])):
	name_obs = os.path.basename(obs_pred[i])
    name_pred = os.path.basename(obs_pred[i-1])
    name_file =  name_obs+'_'+name_pred
    obs_processing = io.imread(each_obs)
    pred_processing = io.imread(each_pred)
    compare_images(obs_processing,pred_processing,str(name_file))
