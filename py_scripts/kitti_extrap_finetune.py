...
orig_weights_file = os.path.join(WEIGHTS_DIR, 'prednet_kitti_weights_teste_t1.hdf5')  # original t+1 weights
orig_json_file = os.path.join(WEIGHTS_DIR, 'prednet_kitti_model_teste_t1.json')

save_model = True
extrap_weights_file = os.path.join(WEIGHTS_DIR, 'prednet_kitti_weights-extrapfinetuned_a3_t5.hdf5')  # where new weights will be saved
extrap_json_file = os.path.join(WEIGHTS_DIR, 'prednet_kitti_model-extrapfinetuned_a3_t5.json')
...
N_seq_val = 306  # number of sequences to use for validation
